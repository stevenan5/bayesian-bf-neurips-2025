...
class BalsubramaniFreund(BaseLabelModel):
    def __init__(self,
        ...
        self.param_eps = None
        # the convex optimzation problem for BF
        self.prob = None
    ...
    def _get_prob_bounds(self, dataset, method='binomial', bound_scale=1):
        ...
        if method != 'unsupervised':
            class_freq_probs[1, :] = np.bincount(y, minlength=self.n_class) / n_labeled
        else:
    ...
    def _make_confidence_progs(self, z, constrs, n_points, bf_preds=None):
        ...
        # make the objective cross entropy -z^T log(bf_pred) if a prediction
        # is given
        if bf_preds is not None:
            obj_fn = -1 * cp.multiply(z, cp.log(bf_preds))
        # otherwise make it z
        else:
            obj_fn = z
    ...
    def _predicted_prob_selections(self, preds, L, prediction_thresholds,
        ...
        # group the datapoints based on the predicted probabilities of BF
    ...
    def get_confidences(self, data, grouping="pattern_neigh",
            neighborhood_size = 5, prediction_thresholds=[], bf_preds=None):
        ...
        # record the mean prediction by BF and from the GT for the groups
        if use_xent:
        ...
        self._make_confidence_progs(z, constrs, n_pts, bf_preds=bf_preds)
